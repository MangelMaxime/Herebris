// ts2fable 0.8.0
module rec ServeServeStaticCore
...
//type RangeParserOptions = Range_parser.Options
//type RangeParserResult = Range_parser.Result
type ParsedQs = Qs.QueryString.ParsedQs
...
//    [<Emit "$0.Invoke('router')">] abstract Invoke_router: unit -> unit
...
type RequestHandler =
    RequestHandler<ParamsDictionary, obj option, obj option, ParsedQs>

type RequestHandler<'P> =
    RequestHandler<'P, obj option, obj option, ParsedQs>

type RequestHandler<'P, 'ResBody> =
    RequestHandler<'P, 'ResBody, obj option, ParsedQs>

type RequestHandler<'P, 'ResBody, 'ReqBody> =
    RequestHandler<'P, 'ResBody, 'ReqBody, ParsedQs>

type RequestHandler<'P, 'ResBody, 'ReqBody, 'ReqQuery> =
    Request<'P, 'ResBody, 'ReqBody, 'ReqQuery> -> Response<'ResBody> -> Func<obj option, unit> -> unit
//    Func<Request<'P, 'ResBody, 'ReqBody, 'ReqQuery>, Response<'ResBody>, NextFunction> -> unit
//    [<Emit "$0($1...)">] abstract Invoke: req: Request<'P, 'ResBody, 'ReqBody, 'ReqQuery> * res: Response<'ResBody> * next: NextFunction -> unit

type ErrorRequestHandler =
    ErrorRequestHandler<ParamsDictionary, obj option, obj option, ParsedQs>

type ErrorRequestHandler<'P> =
    ErrorRequestHandler<'P, obj option, obj option, ParsedQs>

type ErrorRequestHandler<'P, 'ResBody> =
    ErrorRequestHandler<'P, 'ResBody, obj option, ParsedQs>

type ErrorRequestHandler<'P, 'ResBody, 'ReqBody> =
    ErrorRequestHandler<'P, 'ResBody, 'ReqBody, ParsedQs>

type [<AllowNullLiteral>] ErrorRequestHandler<'P, 'ResBody, 'ReqBody, 'ReqQuery> =
    [<Emit "$0($1...)">] abstract Invoke: err: obj option * req: Request<'P, 'ResBody, 'ReqBody, 'ReqQuery> * res: Response<'ResBody> * next: NextFunction -> unit

type PathParams =
    U3<string, RegExp, Array<U2<string, RegExp>>>

type RequestHandlerParams =
    RequestHandlerParams<ParamsDictionary, obj option, obj option, ParsedQs>

type RequestHandlerParams<'P> =
    RequestHandlerParams<'P, obj option, obj option, ParsedQs>

type RequestHandlerParams<'P, 'ResBody> =
    RequestHandlerParams<'P, 'ResBody, obj option, ParsedQs>

type RequestHandlerParams<'P, 'ResBody, 'ReqBody> =
    RequestHandlerParams<'P, 'ResBody, 'ReqBody, ParsedQs>

type RequestHandlerParams<'P, 'ResBody, 'ReqBody, 'ReqQuery> =
    U3<RequestHandler<'P, 'ResBody, 'ReqBody, 'ReqQuery>, ErrorRequestHandler<'P, 'ResBody, 'ReqBody, 'ReqQuery>, Array<U2<RequestHandler<'P>, ErrorRequestHandler<'P>>>>

type IRouterMatcher<'T> =
    IRouterMatcher<'T, obj option>

type [<AllowNullLiteral>] IRouterMatcher<'T, 'Method> =
    [<Emit "$0($1...)">] abstract Invoke: path: PathParams * [<ParamArray>] handlers: Array<RequestHandler<'P, 'ResBody, 'ReqBody, 'ReqQuery>> -> 'T
    [<Emit "$0($1...)">] abstract Invoke: path: PathParams * [<ParamArray>] handlers: Array<RequestHandlerParams<'P, 'ResBody, 'ReqBody, 'ReqQuery>> -> 'T
...
type [<AllowNullLiteral>] IRouterHandler<'T> =
    ...
    [<Emit "$0($1...)">] abstract Invoke: [<ParamArray>] handlers: Array<RequestHandler<'P, 'ResBody, 'ReqBody, 'ReqQuery>> -> 'T
    [<Emit "$0($1...)">] abstract Invoke: [<ParamArray>] handlers: Array<RequestHandlerParams<'P, 'ResBody, 'ReqBody, 'ReqQuery>> -> 'T
...
//    inherit RequestHandler
    ...
    abstract get: path: string * Func<Request, Response, NextFunction, unit> -> unit
    abstract get: path: RegExp * Func<Request, Response, NextFunction, unit> -> unit
    // abstract all: IRouterMatcher<IRouter, string> with get, set
    abstract all: path: string * Func<Request, Response, NextFunction, unit> -> unit
    abstract all: path: RegExp * Func<Request, Response, NextFunction, unit> -> unit
    // abstract post: IRouterMatcher<IRouter, string> with get, set
    abstract post: path: string * Func<Request, Response, NextFunction, unit> -> unit
    abstract post: path: RegExp * Func<Request, Response, NextFunction, unit> -> unit
    // abstract put: IRouterMatcher<IRouter, string> with get, set
    abstract put: path: string * Func<Request, Response, NextFunction, unit> -> unit
    abstract put: path: RegExp * Func<Request, Response, NextFunction, unit> -> unit
    // abstract delete: IRouterMatcher<IRouter, string> with get, set
    abstract delete: path: string * Func<Request, Response, NextFunction, unit> -> unit
    abstract delete: path: RegExp * Func<Request, Response, NextFunction, unit> -> unit
    // abstract patch: IRouterMatcher<IRouter, string> with get, set
    abstract patch: path: string * Func<Request, Response, NextFunction, unit> -> unit
    abstract patch: path: RegExp * Func<Request, Response, NextFunction, unit> -> unit
    // abstract options: IRouterMatcher<IRouter, string> with get, set
    abstract options: path: string * Func<Request, Response, NextFunction, unit> -> unit
    abstract options: path: RegExp * Func<Request, Response, NextFunction, unit> -> unit
    // abstract head: IRouterMatcher<IRouter, string> with get, set
    abstract head: path: string * Func<Request, Response, NextFunction, unit> -> unit
    abstract head: path: RegExp * Func<Request, Response, NextFunction, unit> -> unit
...
type [<AllowNullLiteral>] RequestRanges =
    inherit RangeParser.RangeParser.Ranges

type [<AllowNullLiteral>] Errback =
    [<Emit "$0($1...)">] abstract Invoke: err: Error -> unit

type Request =
    Request<ParamsDictionary, obj option, obj option, ParsedQs>

type Request<'P> =
    Request<'P, obj option, obj option, ParsedQs>

type Request<'P, 'ResBody> =
    Request<'P, 'ResBody, obj option, ParsedQs>

type Request<'P, 'ResBody, 'ReqBody> =
    Request<'P, 'ResBody, 'ReqBody, ParsedQs>
...
type [<AllowNullLiteral>] Request<'P, 'ResBody, 'ReqBody, 'ReqQuery> =
    ...
    abstract range: size: float * ?options: RangeParser.RangeParser.Options -> U2<RangeParser.RangeParser.Range, RangeParser.RangeParser.Result> option
    ...
    abstract res: Response<'ResBody> option with get, set
    abstract next: ?err: obj * unit
...
type Response =
    Response<obj option, float>

type Response<'ResBody> =
    Response<'ResBody, float>

type [<AllowNullLiteral>] Response<'ResBody, 'StatusCode> =
    inherit Http.ServerResponse
    inherit Express.Response
    /// Set status `code`.
    abstract status: code: 'StatusCode -> Response<'ResBody, 'StatusCode>
    /// Set the response HTTP status code to `statusCode` and send its string representation as the response body.
    abstract sendStatus: code: 'StatusCode -> Response<'ResBody, 'StatusCode>
    ...
    ///     });
    abstract links: links: obj option -> Response<'ResBody, 'StatusCode>
    /// Send a response.
    ///
    /// Examples:
    ///
    ...
    abstract send: Send<'ResBody, Response<'ResBody, 'StatusCode>> with get, set
    /// Send JSON response.
    ///
    /// Examples:
    ///
    ...
    abstract json: Send<'ResBody, Response<'ResBody, 'StatusCode>> with get, set
    ...
    abstract jsonp: Send<'ResBody, Response<'ResBody, 'StatusCode>> with get, set
    ...
    ///      res.type('png');
    abstract contentType: ``type``: string -> Response<'ResBody, 'StatusCode>
    /// Set _Content-Type_ response header with `type` through `mime.lookup()`
    /// when it does not contain "/", or set the Content-Type to `type` otherwise.
    ///
    /// Examples:
    ...
    ///      res.type('application/json');
    ///      res.type('png');
    abstract ``type``: ``type``: string -> Response<'ResBody, 'StatusCode>
    ...
    /// instead.
    abstract format: obj: obj option -> Response<'ResBody, 'StatusCode>
    /// Set _Content-Disposition_ header to _attachment_ with optional `filename`.
    abstract attachment: ?filename: string -> Response<'ResBody, 'StatusCode>
    ...
    abstract set: field: obj option -> Response<'ResBody, 'StatusCode>
    abstract set: field: string * ?value: U2<string, ResizeArray<string>> -> Response<'ResBody, 'StatusCode>
    abstract header: field: obj option -> Response<'ResBody, 'StatusCode>
    abstract header: field: string * ?value: U2<string, ResizeArray<string>> -> Response<'ResBody, 'StatusCode>
    abstract headersSent: bool with get, set
    /// Get value for header `field`.
    abstract get: field: string -> string
    /// Clear cookie `name`.
    abstract clearCookie: name: string * ?options: obj -> Response<'ResBody, 'StatusCode>
    ...
    abstract cookie: name: string * ``val``: string * options: CookieOptions -> Response<'ResBody, 'StatusCode>
    abstract cookie: name: string * ``val``: obj option * options: CookieOptions -> Response<'ResBody, 'StatusCode>
    abstract cookie: name: string * ``val``: obj option -> Response<'ResBody, 'StatusCode>
    ...
    ///
    ///       res.location('/login');
    abstract location: url: string -> Response<'ResBody, 'StatusCode>
...
//    abstract locals: Record<string, obj option> with get, set
    abstract charset: string with get, set
    /// Adds the field to the Vary response header, if it is not there already.
    /// Examples:
    ///
    ///      res.vary('User-Agent').render('docs');
    abstract vary: field: string -> Response<'ResBody, 'StatusCode>
    abstract app: Application with get, set
    ...
    abstract append: field: string * ?value: U2<ResizeArray<string>, string> -> Response<'ResBody, 'StatusCode>
...
type RequestParamHandler =
    Func<Request, Response, NextFunction, obj option, string, obj option>
...
//    abstract get: obj with get, set
    ...
    abstract listen: port: float * hostname: string * backlog: float * ?callback: (unit -> unit) -> Http.Server
    abstract listen: port: float * hostname: string * ?callback: (unit -> unit) -> Http.Server
    abstract listen: port: float * ?callback: (unit -> unit) -> Http.Server
...
//    abstract locals: Record<string, obj option> with get, set
    ...
    abstract on: (string -> (Application -> unit) -> Application) with get, set
